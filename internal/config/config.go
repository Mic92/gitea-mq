...
func Load() (*Config, error) {
	...
	cfg.BatchMax, err = parseNonNegInt("GITEA_MQ_BATCH_MAX", 1)
	if err != nil {
		return nil, err
	}
	cfg.BisectMaxSteps, err = parseNonNegInt("GITEA_MQ_BISECT_MAX_STEPS", 0)
	if err != nil {
		return nil, err
	}
...
func parseNonNegInt(envKey string, defaultVal int) (int, error) {
	s := os.Getenv(envKey)
	if s == "" {
	...
	if n < 0 {
		return 0, fmt.Errorf("%s: must be >= 0, got %d", envKey, n)
	}
	return n, nil
}
