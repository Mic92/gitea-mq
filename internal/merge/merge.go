...
func StartTesting(ctx context.Context, giteaClient gitea.Client, svc *queue.Service, owner, repo string, repoID int64, entry *pg.QueueEntry) (*StartTestingResult, error) {
	...
	if err != nil {
		if gitea.IsMergeConflict(err) {
			...
			_ = giteaClient.CreateCommitStatus(ctx, owner, repo, entry.PrHeadSha, gitea.CommitStatus{
				Context:     "gitea-mq",
				State:       "failure",
				Description: "Merge conflict with target branch",
			})
	...
	_ = giteaClient.CreateCommitStatus(ctx, owner, repo, entry.PrHeadSha, gitea.CommitStatus{
		Context:     "gitea-mq",
		State:       "pending",
		Description: "Testing merge result",
	})
