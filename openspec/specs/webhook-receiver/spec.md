...
The system SHALL determine the repository from the webhook payload and route events to the correct repository's queue. Webhooks for repositories not managed by this instance SHALL be ignored.

#### Scenario: Event for a managed repo
- **WHEN** a webhook arrives for repo `org/app` which is configured in this instance
- **THEN** the event is routed to `org/app`'s check monitoring handler

#### Scenario: Event for an unmanaged repo
- **WHEN** a webhook arrives for repo `org/other` which is NOT configured in this instance
- **THEN** the system responds with HTTP 200 but takes no action
- **AND** logs a warning about the unrecognized repository
